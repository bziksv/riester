...
(function (exports,ui_vue3) {
  ...
  })(exports.NavigationFailureType || (exports.NavigationFailureType = {})); // DEV only debug messages
  ...
  function onBeforeRouteLeave(leaveGuard) {
    ...
    const activeRecord = ui_vue3.inject(matchedRouteKey, // to avoid warning
    {}).value;
  ...
  function useLink(props) {
    ...
    const route = ui_vue3.computed(() => router.resolve(ui_vue3.unref(props.to)));
    ...
    function navigate(e = {}) {
      if (guardEvent(e)) {
        return router[ui_vue3.unref(props.replace) ? 'replace' : 'push'](ui_vue3.unref(props.to) // avoid uncaught errors are they are logged anyway
        ).catch(noop);
      }
    ...
    const router = {
      ...
      install(app) {
        ...
        for (const key in START_LOCATION_NORMALIZED) {
          ...
          reactiveRoute[key] = ui_vue3.computed(() => currentRoute.value[key]);
        }

        app.provide(routerKey, router);
        app.provide(routeLocationKey, ui_vue3.reactive(reactiveRoute));
