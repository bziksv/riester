...
class App
{
	...
	public static function onRestAppDelete($app)
	{
		...
		)
		{
			try
			{
				...
				if (!Mask\ItemTable::getList(['filter' => ['=OWNER_TYPE' => Mask\Owner\RestApp::class], 'limit' => 1])->fetch())
				{
	...
	public static function sendRestStatistic()
	{
		...
		{
			$dbRes = Mask\ItemToFileTable::getList([
				'select' => ['APP_ID' => 'ITEM.OWNER_ID', 'CNT'],
				'filter' => [
					'=ITEM.OWNER_TYPE' => Mask\Owner\RestApp::class,
				],
				'runtime' => [new Main\Entity\ExpressionField('CNT', 'COUNT("ID")')],
				'group' => ['ITEM.OWNER_ID'],
			]);
