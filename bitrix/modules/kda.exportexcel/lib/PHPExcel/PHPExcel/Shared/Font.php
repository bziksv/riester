...
/**
 * KDAPHPExcel_Shared_Font
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_Shared
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Shared_Font
{
	 ...
	 *
	 * @param 	KDAPHPExcel_Style_Font			$font			Font object
	 * @param 	KDAPHPExcel_RichText|string	$cellText		Text to calculate width
	 * @param 	integer						$rotation		Rotation angle
	 * @param 	KDAPHPExcel_Style_Font|NULL	$defaultFont	Font object
	 * @return 	integer		Column width
	 */
	public static function calculateColumnWidth(KDAPHPExcel_Style_Font $font, $cellText = '', $rotation = 0, KDAPHPExcel_Style_Font $defaultFont = null) {

		// If it is rich text, use plain text
		if ($cellText instanceof KDAPHPExcel_RichText) {
		...
		try {
			...
			if (self::$autoSizeMethod == self::AUTOSIZE_METHOD_APPROX) {
				throw new KDAPHPExcel_Exception('AutoSize method is set to approx');
			}
		...
		$columnWidth = KDAPHPExcel_Shared_Drawing::pixelsToCellDimension($columnWidth, $defaultFont);
	 ...
	 *
	 * @param string $text
	 * @param KDAPHPExcel_Style_Font
	 * @param int $rotation
	 * @return int
	 * @throws KDAPHPExcel_Exception
	 */
	public static function getTextWidthPixelsExact($text, KDAPHPExcel_Style_Font $font, $rotation = 0) {
		if (!function_exists('imagettfbbox')) {
			throw new KDAPHPExcel_Exception('GD library needs to be enabled');
		}
	 ...
	 */
	public static function getTextWidthPixelsApprox($columnText, KDAPHPExcel_Style_Font $font = null, $rotation = 0)
	{
		...
		switch ($fontName) {
			case 'Calibri':
				...
				$columnWidth = (int) (8.26 * KDAPHPExcel_Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 11; // extrapolate from font size
				break;

			case 'Arial':
				// value 7 was found via interpolation by inspecting real Excel files with Arial 10 font.
				$columnWidth = (int) (7 * KDAPHPExcel_Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 10; // extrapolate from font size
				break;

			case 'Verdana':
				// value 8 was found via interpolation by inspecting real Excel files with Verdana 10 font.
				$columnWidth = (int) (8 * KDAPHPExcel_Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 10; // extrapolate from font size
				break;
	...
	public static function getTrueTypeFontFileFromFont($font) {
		if (!file_exists(self::$trueTypeFontPath) || !is_dir(self::$trueTypeFontPath)) {
			throw new KDAPHPExcel_Exception('Valid directory to TrueType Font files not specified');
		}
		...
		switch ($name) {
			...
			default:
				throw new KDAPHPExcel_Exception('Unknown font name "'. $name .'". Cannot map to TrueType font file');
				break;
		}
		...
		if (!file_exists($fontFile)) {
			throw New KDAPHPExcel_Exception('TrueType Font file not found');
		}
	 ...
	 *
	 * @param KDAPHPExcel_Style_Font $font The workbooks default font
	 * @param boolean $pPixels true = return column width in pixels, false = return in OOXML units
	 * @return mixed Column width
	 */
	public static function getDefaultColumnWidthByFont(KDAPHPExcel_Style_Font $font, $pPixels = false)
	{
	 ...
	 */
	public static function getDefaultRowHeightByFont(KDAPHPExcel_Style_Font $font)
	{
		switch ($font->getName()) {
			case 'Arial':
				switch ($font->getSize()) {
