...
/**
 * KDAPHPExcel_Writer_Excel2007_Chart
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Writer_Excel2007_Chart extends KDAPHPExcel_Writer_Excel2007_WriterPart
{
	/**
	 ...
	 */
	public function writeChart(KDAPHPExcel_Chart $pChart = null)
	{
		// Create XML writer
		$objWriter = null;
	 ...
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	private function _writeTitle(KDAPHPExcel_Chart_Title $title = null, $objWriter)
	{
		if (is_null($title)) {
			return;
		}
	 ...
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	private function _writeLegend(KDAPHPExcel_Chart_Legend $legend = null, $objWriter)
	{
		if (is_null($legend)) {
			return;
		}
	 ...
	 *
	 * @param	KDAPHPExcel_Chart_PlotArea		$plotArea
	 * @param	KDAPHPExcel_Chart_Title		$xAxisLabel
	 * @param	KDAPHPExcel_Chart_Title		$yAxisLabel
	 * @param 	KDAPHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	private function _writePlotArea(KDAPHPExcel_Chart_PlotArea $plotArea,
									KDAPHPExcel_Chart_Title $xAxisLabel = NULL,
									KDAPHPExcel_Chart_Title $yAxisLabel = NULL,
									$objWriter,
									KDAPHPExcel_Worksheet $pSheet)
	{
		if (is_null($plotArea)) {
			return;
		}
				...
				$objWriter->startElement('c:'.$chartType);
					...
					for($i = 0; $i < $groupCount; ++$i) {
						...
						if ($groupType == $chartType) {
							...
							if ($groupType === KDAPHPExcel_Chart_DataSeries::TYPE_RADARCHART) {
								$objWriter->startElement('c:radarStyle');
									$objWriter->writeAttribute('val', $plotStyle );
								$objWriter->endElement();
							} elseif ($groupType === KDAPHPExcel_Chart_DataSeries::TYPE_SCATTERCHART) {
					...
					if ($chartType === KDAPHPExcel_Chart_DataSeries::TYPE_LINECHART) {
						//	Line only, Line3D can't be smoothed

						$objWriter->startElement('c:smooth');
							$objWriter->writeAttribute('val', (integer) $plotGroup->getSmoothLine() );
						$objWriter->endElement();
					} elseif (($chartType === KDAPHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
						($chartType === KDAPHPExcel_Chart_DataSeries::TYPE_BARCHART_3D)) {
						...
						if ($plotGroupingType == 'percentStacked' ||
					...
					} elseif ($chartType === KDAPHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
							...
							$objWriter->startElement('c:showNegBubbles');
								$objWriter->writeAttribute('val', 0 );
							$objWriter->endElement();
					} elseif ($chartType === KDAPHPExcel_Chart_DataSeries::TYPE_STOCKCHART) {
...
//					} while ($id1 == $id2);
					...
					if (($chartType !== KDAPHPExcel_Chart_DataSeries::TYPE_PIECHART) &&
						($chartType !== KDAPHPExcel_Chart_DataSeries::TYPE_PIECHART_3D) &&
						($chartType !== KDAPHPExcel_Chart_DataSeries::TYPE_DONUTCHART)) {
						...
						if ($chartType === KDAPHPExcel_Chart_DataSeries::TYPE_DONUTCHART) {
			...
			if (($chartType !== KDAPHPExcel_Chart_DataSeries::TYPE_PIECHART) &&
				...
				if ($chartType === KDAPHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
	 ...
	 *
	 * @param 	KDAPHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @param 	KDAPHPExcel_Chart_Layout		$chartLayout	Chart layout
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	 ...
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	private function _writeCatAx($objWriter, KDAPHPExcel_Chart_PlotArea $plotArea, $xAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
										$objWriter->writeRawData(KDAPHPExcel_Shared_String::ControlCharacterPHP2OOXML( $caption ));
									$objWriter->endElement();
	 ...
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	private function _writeValAx($objWriter, KDAPHPExcel_Chart_PlotArea $plotArea, $yAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
					...
					if ($groupType !== KDAPHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
	...
	private static function _getChartType($plotArea)
	{
		...
		if ($groupCount == 1) {
			...
			if (count($chartTypes) == 0) {
				throw new KDAPHPExcel_Writer_Exception('Chart is not yet implemented');
			}
		}
	 ...
	 *
	 * @param	KDAPHPExcel_Chart_DataSeries		$plotGroup
									...
									)
	{
		...
		if (($groupType == KDAPHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
			($groupType == KDAPHPExcel_Chart_DataSeries::TYPE_BARCHART_3D)) {
		...
		if (($groupType !== KDAPHPExcel_Chart_DataSeries::TYPE_RADARCHART) &&
			($groupType !== KDAPHPExcel_Chart_DataSeries::TYPE_STOCKCHART)) {

			if ($groupType !== KDAPHPExcel_Chart_DataSeries::TYPE_LINECHART) {
				if (($groupType == KDAPHPExcel_Chart_DataSeries::TYPE_PIECHART) ||
					($groupType == KDAPHPExcel_Chart_DataSeries::TYPE_PIECHART_3D) ||
					($groupType == KDAPHPExcel_Chart_DataSeries::TYPE_DONUTCHART) ||
					($plotSeriesCount > 1)) {
		...
		foreach($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
			$objWriter->startElement('c:ser');
				...
				if (($groupType === KDAPHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
					($groupType === KDAPHPExcel_Chart_DataSeries::TYPE_BARCHART_3D) ||
					($groupType === KDAPHPExcel_Chart_DataSeries::TYPE_BUBBLECHART)) {
				...
				if ($plotSeriesCategory && ($plotSeriesCategory->getPointCount() > 0)) {
					...
					if (($groupType === KDAPHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) ||
						($groupType === KDAPHPExcel_Chart_DataSeries::TYPE_SCATTERCHART)) {
	 ...
	 *
	 * @param	KDAPHPExcel_Chart_DataSeriesValues		$plotSeriesLabel
	 * @param 	KDAPHPExcel_Shared_XMLWriter 			$objWriter 			XML Writer
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	 ...
	 *
	 * @param	KDAPHPExcel_Chart_DataSeriesValues		$plotSeriesValues
	 * @param 	KDAPHPExcel_Shared_XMLWriter 			$objWriter 			XML Writer
	 * @param	string								$groupType			Type of plot for dataseries
	 ...
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	private function _writeBubbles($plotSeriesValues, $objWriter, KDAPHPExcel_Worksheet $pSheet)
	{
		if (is_null($plotSeriesValues)) {
			return;
		}
	 ...
	 * Write Layout
	 *
	 * @param	KDAPHPExcel_Chart_Layout		$layout
	 * @param 	KDAPHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	private function _writeLayout(KDAPHPExcel_Chart_Layout $layout = NULL, $objWriter)
	{
