...
/**
 * KDAPHPExcel_Writer_Excel2007_ContentTypes
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Writer_Excel2007_ContentTypes extends KDAPHPExcel_Writer_Excel2007_WriterPart
{
	/**
	 ...
	 */
	public function writeContentTypes(KDAPHPExcel $pKDAPHPExcel = null, $includeCharts = FALSE)
	{
		// Create XML writer
		$objWriter = null;
		...
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/package/2006/content-types');
			...
			$chart = 1;
			for ($i = 0; $i < $sheetCount; ++$i) {
				$drawings = $pKDAPHPExcel->getSheet($i)->getDrawingCollection();
				$drawingCount = count($drawings);
				$chartCount = ($includeCharts) ? $pKDAPHPExcel->getSheet($i)->getChartCount() : 0;
			...
			// Comments
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($pKDAPHPExcel->getSheet($i)->getComments()) > 0) {
					$this->_writeOverrideContentType(
			...
			for ($i = 0; $i < $mediaCount; ++$i) {
				...
				if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof KDAPHPExcel_Worksheet_Drawing) {
					$extension 	= strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
					$mimeType 	= $this->_getImageMimeType( $this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath() );
				} else if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof KDAPHPExcel_Worksheet_MemoryDrawing) {
			...
			$sheetCount = $pKDAPHPExcel->getSheetCount();
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($pKDAPHPExcel->getSheet()->getHeaderFooter()->getImages()) > 0) {
					foreach ($pKDAPHPExcel->getSheet()->getHeaderFooter()->getImages() as $image) {
	...
	{
		if (KDAPHPExcel_Shared_File::file_exists($pFile)) {
			$image = getimagesize($pFile);
			return image_type_to_mime_type($image[2]);
		} else {
			throw new KDAPHPExcel_Writer_Exception("File $pFile does not exist");
		}
	}
	 ...
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	private function _writeDefaultContentType(KDAPHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
	 ...
	 * @throws 	KDAPHPExcel_Writer_Exception
	 */
	private function _writeOverrideContentType(KDAPHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
		if ($pPartname != '' && $pContentType != '') {
			// Write content type
