...
/**
 * KDAPHPExcel_Writer_Excel5
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Writer_Excel5 extends KDAPHPExcel_Writer_Abstract implements KDAPHPExcel_Writer_IWriter
{
	/**
	 * KDAPHPExcel object
	 *
	...
	/**
	 * Create a new KDAPHPExcel_Writer_Excel5
	 *
	...
	public function __construct(KDAPHPExcel $phpExcel) {
		...
		$this->_parser		= new KDAPHPExcel_Writer_Excel5_Parser();
	}
	...
	public function save($pFilename = null) {
		...
		$this->_writerWorkbook = new KDAPHPExcel_Writer_Excel5_Workbook($this->_phpExcel,
		...
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writerWorksheets[$i] = new KDAPHPExcel_Writer_Excel5_Worksheet($this->_str_total, $this->_str_unique,
		...
		$OLE = new KDAPHPExcel_Shared_OLE_PPS_File(KDAPHPExcel_Shared_OLE::Asc2Ucs($workbookStreamName));
		...
		if(isset($this->_documentSummaryInformation) && !empty($this->_documentSummaryInformation)){
			$OLE_DocumentSummaryInformation = new KDAPHPExcel_Shared_OLE_PPS_File(KDAPHPExcel_Shared_OLE::Asc2Ucs(chr(5) . 'DocumentSummaryInformation'));
		...
		if(isset($this->_summaryInformation) && !empty($this->_summaryInformation)){
		  $OLE_SummaryInformation = new KDAPHPExcel_Shared_OLE_PPS_File(KDAPHPExcel_Shared_OLE::Asc2Ucs(chr(5) . 'SummaryInformation'));
		...
		$root = new KDAPHPExcel_Shared_OLE_PPS_Root(time(), time(), $arrRootData);
		// save the OLE file
	 ...
	 * @return KDAPHPExcel_Writer_Excel5
	 */
	...
	private function _buildWorksheetEschers()
	{
		...
		foreach ($this->_phpExcel->getAllsheets() as $sheet) {
			...
			$escher = new KDAPHPExcel_Shared_Escher();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				...
				$twoAnchor = KDAPHPExcel_Shared_Excel5::oneAnchor2twoAnchor($sheet, $coordinates, $offsetX, $offsetY, $width, $height);
			...
			if(!empty($filterRange)){
				$rangeBounds = KDAPHPExcel_Cell::rangeBoundaries($filterRange);
				...
				while($iInc <= $iNumColEnd){
					...
					$oDrawing  = new KDAPHPExcel_Worksheet_BaseDrawing();
					// get the coordinates of drawing
					$cDrawing   = KDAPHPExcel_Cell::stringFromColumnIndex($iInc - 1) . $rangeBounds[0][1];
					...
					// set coordinates and offsets, client anchor
					$endCoordinates = KDAPHPExcel_Cell::stringFromColumnIndex(KDAPHPExcel_Cell::stringFromColumnIndex($iInc - 1));
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				if ($drawing instanceof KDAPHPExcel_Worksheet_Drawing) {
					...
					case 1: // GIF, not supported by BIFF8, we convert to PNG
						$blipType = KDAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
					...
					case 2: // JPEG
						$blipType = KDAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
					...
					case 6: // Windows DIB (BMP), we convert to PNG
						$blipType = KDAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
						imagepng(KDAPHPExcel_Shared_Drawing::imagecreatefrombmp($filename));
						$blipData = ob_get_contents();
						ob_end_clean();
						break;
				...
				} else if ($drawing instanceof KDAPHPExcel_Worksheet_MemoryDrawing) {

					switch ($drawing->getRenderingFunction()) {

					case KDAPHPExcel_Worksheet_MemoryDrawing::RENDERING_JPEG:
						$blipType = KDAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
						$renderingFunction = 'imagejpeg';
						break;

					case KDAPHPExcel_Worksheet_MemoryDrawing::RENDERING_GIF:
					case KDAPHPExcel_Worksheet_MemoryDrawing::RENDERING_PNG:
					case KDAPHPExcel_Worksheet_MemoryDrawing::RENDERING_DEFAULT:
						$blipType = KDAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
	...
	private function _writeSummaryInformation(){
		...
		if($this->_phpExcel->getProperties()->getCreated()){
			...
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0C),
								   ...
								   'data'	=> array('data' => KDAPHPExcel_Shared_OLE::LocalDate2OLE($dataProp)));
			$dataSection_NumProps++;
		}
		//	Modified Date/Time
