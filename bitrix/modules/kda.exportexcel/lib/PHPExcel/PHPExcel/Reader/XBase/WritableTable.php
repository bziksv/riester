...
{    
    ...
    public function create($filename, $fields, $recordFullCount=0)
    {        
		...
		$fileEncoding = $this->getEncoding();
		$siteEncoding = $this->getSiteEncoding();
        foreach ($fields as $field) {
			...
			{
				 $field[0] = \Bitrix\Main\Text\Encoding::convertEncoding($field[0], $siteEncoding, $fileEncoding);
			}
        ...
        $result->recordCount = 0;
		$result->recordFullCount = (int)$recordFullCount;
    ...
    public function openContinueWrite($filename)
    {
    ...
    public function writeHeader()
    {
		...
		$this->writeInt($this->recordFullCount);
