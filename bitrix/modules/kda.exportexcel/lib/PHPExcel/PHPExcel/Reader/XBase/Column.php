...
class Column 
{
    ...
    public function __construct(Table $table, $name, $type, $memAddress, $length, $decimalCount, $reserved1, $workAreaID, $reserved2, $setFields, $reserved3, $indexed, $colIndex, $bytePos) 
    {
		...
		$this->name = ($this->table->strpos($name, 0x00) !== false ) ? $this->table->substr($name, 0, $this->table->strpos($name, 0x00)) : $name;
		if(!($table instanceof WritableTable)) $this->name = ToLower($this->name);
        $this->type = $type;
