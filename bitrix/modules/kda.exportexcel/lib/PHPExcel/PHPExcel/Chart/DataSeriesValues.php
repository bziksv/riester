...
/**
 * KDAPHPExcel_Chart_DataSeriesValues
 *
 * @category	KDAPHPExcel
 * @package		KDAPHPExcel_Chart
 * @copyright	Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Chart_DataSeriesValues
{
	...
	/**
	 * Create a new KDAPHPExcel_Chart_DataSeriesValues object
	 */
	 ...
	 *								Typical values are:
	 *									KDAPHPExcel_Chart_DataSeriesValues::DATASERIES_TYPE_STRING
	 *										Normally used for axis point values
	 *									KDAPHPExcel_Chart_DataSeriesValues::DATASERIES_TYPE_NUMBER
	 *										Normally used for chart data values
	 * @return	KDAPHPExcel_Chart_DataSeriesValues
	 */
	public function setDataType($dataType = self::DATASERIES_TYPE_NUMBER) {
		if (!in_array($dataType, self::$_dataTypeValues)) {
    		throw new KDAPHPExcel_Chart_Exception('Invalid datatype for chart data series values');
		}
	...
	public function setDataValues($dataValues = array(), $refreshDataSource = TRUE) {
		$this->_dataValues = KDAPHPExcel_Calculation_Functions::flattenArray($dataValues);
	...
	public function refresh(KDAPHPExcel_Worksheet $worksheet, $flatten = TRUE) {
        if ($this->_dataSource !== NULL) {
        	$calcEngine = KDAPHPExcel_Calculation::getInstance($worksheet->getParent());
			$newDataValues = KDAPHPExcel_Calculation::_unwrapResult(
			...
			if ($flatten) {
				$this->_dataValues = KDAPHPExcel_Calculation_Functions::flattenArray($newDataValues);
				...
				$dimensions = KDAPHPExcel_Cell::rangeDimension(str_replace('$','',$cellRange));
