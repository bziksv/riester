...
(function (exports,main_core,landing_node) {
	'use strict';

	class Text extends landing_node.Node {
	  constructor() {
	    super();
	    ...
	    this.onMouseup = this.onMouseup.bind(this); // Bind on node events
	  ...
	  onInput(event) {
	    ...
	    if (this.isTable(event)) {
	      ...
	      if (event.srcElement.textContent === '' && event.srcElement.classList.contains('landing-table-td') && tableFontSize < this.tableBaseFontSize) {
	  ...
	  onMousedown(event) {
	    if (!this.manifest.group) {
	      ...
	      if (this.manifest.allowInlineEdit !== false && BX.Landing.Main.getInstance().isControlsEnabled()) {
	        ...
	        if (this.isTable(event)) {
	          this.disableEdit();
	          BX.Landing.Block.Node.Text.currentNode.node.querySelectorAll('.landing-table-container').forEach(function (table) {
	  ...
	  addTableButtons(event) {
	    ...
	    if (event.srcElement.classList.contains('landing-table') || event.srcElement.classList.contains('landing-table-col-dnd')) {
	  ...
	  getTableButtons() {
	    ...
	    this.buttons.push(new BX.Landing.UI.Button.AlignTable("alignLeft", {
	      ...
	      html: BX.Landing.Loc.getMessage("LANDING_TITLE_OF_EDITOR_ACTION_TABLE_STYLE") + "<i class=\"fas fa-chevron-down g-ml-8\"></i>",
	      attrs: {
...
}((this.BX.Landing.Node = this.BX.Landing.Node || {}),BX,BX.Landing));
