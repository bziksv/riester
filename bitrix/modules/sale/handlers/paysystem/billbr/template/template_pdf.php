...
{
	$pdf->Ln();
	$text = CSalePdf::prepareToPdf(sprintf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_PHONE', null, $billLang).".: %s", $params["SELLER_COMPANY_PHONE"]));
	$textWidth = $width - $logoWidth;
	while ($pdf->GetStringWidth($text))
	{
		list($string, $text) = $pdf->splitString($text, $textWidth);
...
if ($params['BILLBR_PAYER_SHOW'] === 'Y')
{
	...
	$title = CSalePdf::prepareToPdf($params['BILLBR_HEADER'].' '.Loc::getMessage('SALE_HPS_BILLBR_NUMBER', null, $billLang).': ');
...
if ($params['BASKET_ITEMS'])
{
		...
		foreach ($arCols as $columnId => $col)
		{
			...
			switch ($columnId)
			{
				...
				case 'QUANTITY':
					...
					break;
				case 'MEASURE':
					$data = CSalePdf::prepareToPdf($basketItem["MEASURE_NAME"] ? $basketItem["MEASURE_NAME"] : Loc::getMessage('SALE_HPS_BILLBR_MEASURE', null, $billLang));
					$arCols[$columnId]['IS_DIGIT'] = true;
					break;
				case 'PRICE':
	...
	if ($params['BILLBR_TOTAL_SHOW'] == 'Y')
	{
		...
		if ($params['SUM'] - $sum > $eps)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_SUB_TOTAL', null, $billLang).":");
		...
		if ($params['SUM_PAID'])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_SUM_PAID', null, $billLang).":");
		...
		if ($params['DISCOUNT_PRICE'])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_DISCOUNT', null, $billLang).":");
		...
		$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_TOTAL', null, $billLang).":");
...
if ($bankAccNo && $bankRouteNo && $bankSwift)
{
	...
	$bankDetails .= CSalePdf::prepareToPdf(sprintf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK', null, $billLang)." ".Loc::getMessage('SALE_HPS_BILLBR_NUMBER', null, $billLang).": %s\n", $bankAccNo));
	...
	{
		$bankDetails .= CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_2', null, $billLang).": ");
		if ($bank)
	...
	$bankDetails .= CSalePdf::prepareToPdf(sprintf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_ROUTE_NO', null, $billLang).": %s\n", $bankRouteNo));
	$bankDetails .= CSalePdf::prepareToPdf(sprintf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_SWIFT', null, $billLang).": %s\n", $bankSwift));
