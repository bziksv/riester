...
var bx_adv_includeFlash = function(container_id, flash_link, width, height, wmode, img_link, flashVer, altHref, target, title)
{
	...
	if(bx_adv_getFlashVersion() > flashVer)
	{
		...
		obFrame.innerHTML = oeTags;
	} 
	else 
	{
		obFrame.innerHTML = (altHref != ''? '<a href="'+altHref+'" target="'+target+'" title="'+title+'">':'')+'<img src="' + img_link + '" width="' + width + '" heigh="' + height + '" alt="'+title+'" border="0" />'+(altHref != ''? '</a>':'');
	}
}