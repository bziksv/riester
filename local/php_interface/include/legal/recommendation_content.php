...
<div class="legal-doc legal-doc--recommendation">
    ...
    <p>1. <?= legal_var($legal['operator_name']) ?> (ИНН: <?= legal_var($legal['inn']) ?>, ОГРН: <?= legal_var($legal['ogrn']) ?>, КПП: <?= legal_var($legal['kpp']) ?>, адрес: <?= legal_var($legal['address_legal']) ?>), далее – Оператор, является владельцем сайта <?= legal_link($legal['site']) ?> («Сайт»).</p>
    ...
    <ul>
        <?= riesterLegalRenderThirdPartyListItems() ?>
    </ul>
